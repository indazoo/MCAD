...
pitch  = 1;
backlash_input = 0.8941250000000001;
...
function convert_to_lines(profile_pt_map=[]) =
		[	for(i=[0:len(profile_pt_map)-2])
			let(xy = profile_pt_map[i],
				...
				//[xz,xz_next]
				line_build(pt1=xy, pt2=xy_next, line_index = i, source_line_index=undef)
		];
	...
	[ for(i=[0:len(profile_cline_map)-1])
		...
		[ 	//1: Move of main line
			...
			(slope == line_def_slope_horizontal) ? 
				...
				is_needed ?
					line_build(pt1=pt_move_x(point=line_pt_1, move_x=-prev_bckl), 
...
function cline_adjust_index(cline=[], line_index=0, source_line_index=0) =
			line_build(pt1=cline[0], pt2=cline[1],
...
for(t=backlashed_indexed_lines)	echo("	", t);				



for(cline = lines)
	draw_cline(cline, col="yellow");

//for(cline = backlashed_indexed_lines) draw_cline(cline, col="green");
...
lines_crossing = remove_disjoints(line_crossings(clines = backlashed_indexed_lines));
			...
			let(xst_cross = 
				...
				[for(crossed = crossed_lines)
					...
					xst[7] == undef ?
						[]
					:
					xst[7]==crossed[2][7] ? //compare source line id
						crossed[2]
					:
					xst[7]==crossed[3][7] ? //compare source line id
						crossed[3]
					:
						[]
...
// all points in all clines and the first index of this value. 
function clines_find_extreme_y(clines=[], is_outline=true, current_index=0, extreme_y=[0,0]) =
	current_index >= len(clines) ?
		extreme_y
	:
	let(initialized_extreme_y = (current_index == 0 ? clines[0][0].y : extreme_y[0]),
		found_extreme = 
			is_outline ?
				max(clines[current_index][0].y,
					clines[current_index][1].y,
					initialized_extreme_y)
			:
				min(clines[current_index][0].y,
					clines[current_index][1].y,
					initialized_extreme_y)
			,
		found_index = 
			current_index == 0 ? 0 
			: found_extreme == initialized_extreme_y ? extreme_y[1] : current_index
	)
	clines_find_extreme_y(
		clines=clines, 
		is_outline = is_outline,
		current_index=current_index+1, 
		extreme_y= [found_extreme, found_index]
	)
	;
// ------------------------------------------------------------------------------------------
...
function clines_find_connected(clines=[], cline_index = 0, seek_direction = -1,
	...
	len(clines) == 0 ?
		connected_lines
	: num_worked_lines >= len(clines)-2 ? //End/Safety check
		connected_lines
	...
	let(cline = clines[cline_index],
		...
		//is_connected = candidate_index != cline_index &&
...
test_index = 8;//extreme[1];
test_direction = -1;
connected = clines_find_connected(
							clines=existing_lines_with_crossed, 
							cline_index = test_index,
							seek_direction=test_direction);		
echo("connected", len(connected), test_index, existing_lines_with_crossed[test_index]);
for(t=connected) 
{
	echo("	", t);
}	
...
//TODO: there may be a loop in the lines. so we have to take the longer thread in the end
	len(clines) == 0 ?
		clines_sequence
	: num_worked_lines >= len(clines) ? //End/Safety check
		clines_sequence
	:
	let(
		forward_seek_is_done = forward_seek_is_done || (current_index >= len(clines)-1),
		backward_seek_is_done = forward_seek_is_done
								&& (backward_seek_is_done || (current_index <=0)),
		seek_direction = !forward_seek_is_done ? 1
						: !backward_seek_is_done ? -1
						: 0,
		cline_index = num_worked_lines == 0 || 
						(forward_seek_is_done && (current_index == len(clines)-1)) ?
							start_index 
						: current_index,
		cur_cline = clines[cline_index],
		cur_pt1 = cur_cline[0],
		cur_pt2 = cur_cline[1],
	...
	:
		//TODO: decide which of the result is the one which is connected
		let(next_connected = connected_lines[0],
			seq_lines = num_worked_lines == 0 ?
						[cur_cline] : clines_sequence
		)
		clines_find_sequence(
				clines=clines, 
				is_outline=is_outline,
				start_index = start_index, 
				current_index=next_connected[7], 
				num_worked_lines = num_worked_lines + 1, 
				clines_sequence=
					seek_direction > 0 ?
				...
				backward_seek_is_done = backward_seek_is_done
			)

	;
	
function clines_mark_as_used(cline=[]) =
		[
		cline[0],
		cline[1],
		cline[2],
...
function clines_mark_as_unused(cline=[]) =
		[
		cline[0],
		cline[1],
		cline[2],
...
function clines_is_unused(cline=[]) =
	len(cline)<10 || cline[9]==false;
function clines_is_used(cline=[]) =
	len(cline)>=10 && cline[9]==true;
...
t_sequence_max = clines_find_sequence(
							clines=existing_lines_with_crossed, 
							start_index = test_index, 
							is_outline=is_outline);
echo("t_sequence_max", len(t_sequence_max));
for(t=t_sequence_max) 
{
	echo("	",t);
	//fine_xy_point(point = t[1], width = 0.2, col = "red");
...
lines_to_delete = lines_cleanup(clines = existing_lines_with_crossed, is_outline=is_outline);
...
lines_needed = remove_lines(clines = existing_lines_with_crossed, to_delete_clines=lines_to_delete);
...
//          intersection point
//          
function line_2D_cross(line_1, line_2, do_report_line_touches=false) =
	let(line_1_pt_1 = line_1[0],
		...
		result_disjoint = [true, line_def_cross_disjoint,[], line_1, line_2],
		...
		pt2_line_1_touch_line_2 = pt2_line1_is_collin_line2 && pt1_line_1_is_on_line_2 ?
									line_2D_cross_touch_collinear_pt(line=line_2, point=line_1_pt_2, do_report_line_touches = do_report_line_touches)
									:result_disjoint,
		...
		pt2_line_2_touch_line_1 = pt2_line2_is_collin_line1 && pt2_line_2_is_on_line_1 ?
									line_2D_cross_touch_collinear_pt(line=line_1, point=line_2_pt_2, do_report_line_touches = do_report_line_touches)
									:result_disjoint,
		cross_result = (
			line_1_is_a_point && line_2_is_a_point ?
				...
				line_1_line_2_same_spot ?
					do_report_line_touches ? 
						[line_def_cross_intersect, line_1_pt_1, line_1, line_2] //Result: two points at the same place
			...
			//CASE 4: Both lines have two distinct points ==> two lines
				...
				&& (!(max_z_line_1 < min_z_line_2 || max_z_line_2 < min_z_line_1 ))
					...
					//CASE 5.3
						...
						[false, calc_cross[0], calc_cross[1],44, calc_cross] // Result: calculate cross
	...
	:
		[cross_result[1], // cross status
		 cross_result[2], // cross point
		 line_build(line_1_pt_1,cross_result[2],line_1_num, line_1_num),
		 line_build(cross_result[2],line_2_pt_2,line_2_num, line_2_num),
		 cross_result[2], // line
		 cross_result[3],
		 cross_result[4],
		 cross_result[5],
...
function line_2D_cross_touch_collinear_pt(line=[[],[]], point=[], do_report_line_touches = true) =
			...
			do_report_line_touches ?
				[line_def_cross_intersect, point_on_pt1, point_on_pt2] //Result: point is anywhere on line
			:
				...
				: [line_def_cross_intersect, point_on_pt1, point_on_pt2] //Result: point is anywhere on "inside" of line, not on endpoints
			;
